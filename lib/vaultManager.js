...
class VaultManager {
  ...
  async syncItemToCloud(itemId, serverUrl) {
    ...
    const activeServerUrl = serverUrl || (this.indexData.settings && this.indexData.settings.cloudServerUrl) || 'https://win-locker.vercel.app';
    ...
    if (item.type === 'file') {
      ...
      const childFiles = this.indexData.items.filter(i => i.parentId === itemId && i.type === 'file');
      let uploadedCount = 0;
  ...
  async syncAllToCloud(serverUrl) {
    ...
    if (itemsToSync.length === 0) {
      return { success: true, count: 0, message: 'No files to sync' };
    }
