...
class VaultManager {
  ...
  async syncAllToCloud(serverUrl) {
    ...
    // Sync and upload all folder meta
    if (this.indexData.folders) {
      for (const folder of this.indexData.folders) {
        try {
          const metaBuffer = Buffer.from(JSON.stringify({ folderName: folder.name, folderId: folder.id, createdAt: new Date().toISOString() }));
          await this.uploadBufferToCloud(metaBuffer, `${folder.name}.folder_meta`, activeServerUrl);
          folder.isSelectiveBackup = true;
          folder.cloudSyncedAt = new Date().toISOString();
          count++;
        } catch (fErr) {
          console.warn(`Could not sync folder meta for ${folder.name}:`, fErr.message);
        }
      }
    }
    ...
    return { success: true, count, total: itemsToSync.length + (this.indexData.folders ? this.indexData.folders.length : 0), errors };
  }
